...
export default function SearchAppBar({ onSearch }: any) {
  ...
  const handleFormSubmit: SubmitHandler<FieldValues> = (data) => {
    AdvertiseFormService.createAdvertiseForm(
      {
        name: data.name,
      ...
      .then((res) => {
        navigate(`${routes.admin.advertisesForm.root}`);
  ...
  return (
    <Box className={classes["search-wrapper"]}>
      ...
      <DialogWrapper open={isDialogOpen} onClose={handleCloseDialog}>
        <DialogTitle>Thêm hình thức quảng cáo</DialogTitle>
        <FormWrapper {...methods} onSubmit={methods.handleSubmit(handleFormSubmit)}>
          <TextField
            {...methods.register("name", { required: "Tên hình thức báo cáo không được để rỗng" })}
