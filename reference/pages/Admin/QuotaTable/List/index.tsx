...
import { GetQuotaTablesParams, QuotaTable } from "models/Admin/quota";
...
interface Props {
  ...
  data: DataPagination<QuotaTable>,
  setData: React.Dispatch<React.SetStateAction<DataPagination<QuotaTable>>>,
}

const List = memo(({ keyword, setKeyword, data, setData }: Props) => {

  const dispatch = useDispatch()
  const [itemAction, setItemAction] = useState<QuotaTable>();
  const [actionAnchor, setActionAnchor] = useState<null | HTMLElement>(null);
  const [languageAnchor, setLanguageAnchor] = useState<null | HTMLElement>(null);
  const [itemDelete, setItemDelete] = useState<QuotaTable>(null);

  const handleAdd = () => {
    dispatch(push(routes.admin.quotaTable.create));
  }
  ...
  const fetchData = (value?: { take?: number, page?: number, keyword?: string }) => {
    ...
    const params: GetQuotaTablesParams = {
    ...
    AdminQuotaService.getQuotaTables(params)
      .then((res) => {
  ...
  const onDelete = () => {
    ...
    onCloseConfirm()
    dispatch(setLoading(true))
    AdminQuotaService.deleteQuotaTable(itemDelete.id)
      .then(() => {
        fetchData()
      })
      .catch((e) => dispatch(setErrorMess(e)))
  ...
  const onRedirectEdit = (item: QuotaTable, lang?: LangSupport) => {
    dispatch(push({
      pathname: routes.admin.quotaTable.edit.replace(':id', `${item.id}`),
