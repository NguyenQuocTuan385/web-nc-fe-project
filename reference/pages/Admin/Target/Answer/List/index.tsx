...
import { GetAnswersParams, TargetAnswer } from "models/Admin/target";
...
const tableHeaders: TableHeaderLabel[] = [
  ...
  { name: 'group', label: 'Group', sortable: false },
...
const AnswerList = memo(({ }: Props) => {
  ...
  const [data, setData] = useState<DataPagination<TargetAnswer>>();
  const [itemAction, setItemAction] = useState<TargetAnswer>();
  const [actionAnchor, setActionAnchor] = useState<null | HTMLElement>(null);
  const [languageAnchor, setLanguageAnchor] = useState<null | HTMLElement>(null);
  const [itemDelete, setItemDelete] = useState<TargetAnswer>(null);

  const handleAdd = () => {
    dispatch(push(routes.admin.target.question.answer.create.replace(":id", questionId)));
  }
  ...
  const fetchData = (value?: { take?: number, page?: number, keyword?: string }) => {
    ...
    TargetAnswerService.getAnswers(params)
      .then((res) => {
  ...
  const onDelete = () => {
    if (!itemDelete) return
    dispatch(setLoading(true))
    TargetAnswerService.delete(itemDelete.id)
      .then(() => {
        fetchData()
      })
      .catch((e) => dispatch(setErrorMess(e)))
  ...
  const onRedirectEdit = (item: TargetAnswer, lang?: LangSupport) => {
    dispatch(push({
      pathname: routes.admin.target.question.answer.edit.replace(':id', `${questionId}`).replace(':answerId', `${item.id}`),
