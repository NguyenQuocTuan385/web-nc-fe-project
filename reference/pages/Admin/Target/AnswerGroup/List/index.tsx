...
import { GetAnswerGroupsParams, TargetAnswerGroup } from "models/Admin/target";
...
const AnswerGroupList = memo(({ }: Props) => {

  const { id: questionId } = useParams<{ id: string }>();
  const dispatch = useDispatch()
  const [keyword, setKeyword] = useState<string>('');
  const [data, setData] = useState<DataPagination<TargetAnswerGroup>>();
  const [itemAction, setItemAction] = useState<TargetAnswerGroup>();
  const [actionAnchor, setActionAnchor] = useState<null | HTMLElement>(null);
  const [languageAnchor, setLanguageAnchor] = useState<null | HTMLElement>(null);
  const [itemDelete, setItemDelete] = useState<TargetAnswerGroup>(null);

  const handleAdd = () => {
    dispatch(push(routes.admin.target.question.answerGroup.create.replace(":id", questionId)));
  }

  const handleBack = () => {
    dispatch(push(routes.admin.target.question.edit.replace(":id", questionId)));
  }
  ...
  const fetchData = (value?: { take?: number, page?: number, keyword?: string }) => {
    ...
    const params: GetAnswerGroupsParams = {
    ...
    TargetAnswerGroupService.getAnswerGroups(params)
      .then((res) => {
  ...
  const onDelete = () => {
    if (!itemDelete) return
    dispatch(setLoading(true))
    TargetAnswerGroupService.delete(itemDelete.id)
      .then(() => {
        fetchData()
      })
      .catch((e) => dispatch(setErrorMess(e)))
  ...
  const onRedirectEdit = (item: TargetAnswerGroup, lang?: LangSupport) => {
    dispatch(push({
      pathname: routes.admin.target.question.answerGroup.edit.replace(':id', `${questionId}`).replace(':answerGroupId', `${item.id}`),
