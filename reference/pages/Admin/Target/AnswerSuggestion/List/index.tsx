...
import { GetAnswerSuggestionsParams, TargetAnswerSuggestion } from "models/Admin/target";
...
const AnswerSuggestionList = memo(({ }: Props) => {

  const { id: questionId } = useParams<{ id: string }>();
  const dispatch = useDispatch()
  const [keyword, setKeyword] = useState<string>('');
  const [data, setData] = useState<DataPagination<TargetAnswerSuggestion>>();
  const [itemAction, setItemAction] = useState<TargetAnswerSuggestion>();
  const [actionAnchor, setActionAnchor] = useState<null | HTMLElement>(null);
  const [languageAnchor, setLanguageAnchor] = useState<null | HTMLElement>(null);
  const [itemDelete, setItemDelete] = useState<TargetAnswerSuggestion>(null);

  const handleAdd = () => {
    dispatch(push(routes.admin.target.question.answerSuggestion.create.replace(":id", questionId)));
  }
  ...
  const fetchData = (value?: { take?: number, page?: number, keyword?: string }) => {
    ...
    TargetAnswerSuggestionService.getAnswerSuggestions(params)
      .then((res) => {
...
export default AnswerSuggestionList